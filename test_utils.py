    ...
    if isinstance(target_output, dict):
        for key in target_output.keys():
            try:
                ...
                print(
                    f"Error: {error} in variable {key}. Got {type(target_output[key])} but expected type {type(expected_output[key])}")
        ...
        for i in range(len(target_output)):
            try:
                ...
                print(
                    f"Error: {error} in variable {i}, expected type: {type(target_output[i])}  but expected type {type(expected_output[i])}")
